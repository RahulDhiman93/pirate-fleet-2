...
class Player {
    ...
    func addPlayerShipsMinesMonsters(_ numberOfMines: Int = 0, numberOfSeamonsters: Int = 0) {
        ...
        for _ in 0..<numberOfMines {
            ...
            var mine = Mine(guaranteesHit: true, PenaltyText: "BOMB!!", location: location)
            while !gridViewController.addMine(mine, playerType: .computer) {
                location = RandomGridLocation()
                mine = Mine(guaranteesHit: true, PenaltyText: "BOMB!!",location: location)
            }
        }
        ...
        for _ in 0..<numberOfSeamonsters {
            var location = RandomGridLocation()
            var seaMonster = SeaMonster(PenaltyText: "YOU HIT A SEA MONSTER", location: location, guaranteesHit: true)
            while !gridViewController.addSeamonster(seaMonster, playerType: .computer) {
                location = RandomGridLocation()
                seaMonster = SeaMonster(PenaltyText: "YOU HIT A SEA MONSTER",location: location,guaranteesHit: true)
            }
        }
    }
}
